...
=====================
floodprotectfix.patch
=====================

In the legacy code, a client is able to send one reliable command per second.
Examples of reliable commands are ut_itemdrop, chat, and ut_radio.  One of
the problems in the legacy code is that if you keep sending reliable commands
quickly, the one second timer keeps resetting, and none of your reliable
commands get executed, even if you try sending them for a minute or more.
The only way around this is to stop trying to send commands (back off).
This patch fixes that problem.  Also, this patch allows 2 reliable commands
in a period of 1.5 seconds.  So you can, for example, drop a weapon and a
kevlar vest at the same time, instantaneously.  Of course this reliable
command limit is only in effect when sv_floodProtect is on.
